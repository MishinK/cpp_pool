...
void Form::execute(Bureaucrat const &executor) const
{
    ...
    {
        if (!is_signed)
	        throw (Form::UnsignedFormException(name));
        if (execute_grade < executor.getGrade())
	        throw (Form::GradeTooLowException(name));
        std::cout << "Bureaucrat " << executor.getName() << " executes " << name << std::endl;
    }
    catch(const std::exception& e)
    {
        std::cerr << e.what() << '\n';
...
Form::UnsignedFormException::UnsignedFormException(std::string _name) throw():
Exception("\x1b[1;31mException Form: " + _name + ", unsigned form\x1b[0m"){}
